...
def generate_edge_gcode(edge_geom, output_path: str, cfg: Config = None):
    """G-code para troquelado del contorno externo del PCB.

    Traza el exterior del Edge.Cuts offseteado hacia afuera por el radio de la
    fresa, en múltiples pasadas de profundidad hasta EDGE_DEPTH_MM.
    """
    if cfg is None:
        cfg = Config()

    tool_radius = cfg.EDGE_TOOL_MM / 2.0

    # Calcular pasadas de profundidad: -0.5, -1.0, -1.5, -1.7 (ejemplo)
    depths = []
    z = cfg.EDGE_PASS_DEPTH_MM
    while z > cfg.EDGE_DEPTH_MM:
        depths.append(round(z, 6))
        z += cfg.EDGE_PASS_DEPTH_MM
    depths.append(cfg.EDGE_DEPTH_MM)

    # Extraer polígonos del outline y aplicar offset de herramienta hacia afuera
    raw_polys = list(edge_geom.geoms) if isinstance(edge_geom, MultiPolygon) else [edge_geom]
    cut_paths = []
    for poly in raw_polys:
        if not isinstance(poly, Polygon) or poly.is_empty:
            continue
        outer = Polygon(poly.exterior)           # solo contorno exterior
        cut = outer.buffer(tool_radius, join_style=2, cap_style=2)
        if cut.is_empty:
            continue
        for cp in (list(cut.geoms) if isinstance(cut, MultiPolygon) else [cut]):
            if isinstance(cp, Polygon) and not cp.is_empty:
                cut_paths.append(list(cp.exterior.coords))

    if not cut_paths:
        raise ValueError("No se pudo construir el path de troquelado.")

    print(f"[edge] {output_path}  ({len(cut_paths)} contorno(s), {len(depths)} pasada(s)) ...")

    lines = [
        "( Generated by gerber2gcode.py — EDGE CUT )",
        f"( Tool diameter  : {cfg.EDGE_TOOL_MM:.3f} mm )",
        f"( Total depth    : {cfg.EDGE_DEPTH_MM:.3f} mm )",
        f"( Pass depth     : {cfg.EDGE_PASS_DEPTH_MM:.3f} mm )",
        f"( Passes         : {len(depths)} )",
        "G17", "G21", "G54", "G90",
        f"G00 Z{cfg.EDGE_SAFE_Z_MM:.4f}",
    ]
    if cfg.SPINDLE_ON:
        lines.append("S10000 M03")
    ...
    for path in cut_paths:
        ...
        for depth in depths:
            lines += [
                f"G00 X{x0:.3f} Y{y0:.3f}",
                f"G01 F{cfg.EDGE_PLUNGE_RATE} Z{depth:.4f}",
            ]
            for x, y in path[1:]:
                lines.append(f"G01 F{cfg.EDGE_FEED_RATE} X{x:.3f} Y{y:.3f}")
            lines.append(f"G01 F{cfg.EDGE_FEED_RATE} X{x0:.3f} Y{y0:.3f}")  # cierre
            lines.append(f"G00 Z{cfg.EDGE_SAFE_Z_MM:.4f}")
...
def run(gbr_path: str, output_path: str, drl_paths: list = None,
    ...
    if edge_path:
        (progress_cb or print)(f"[edge] Procesando Edge.Cuts: {edge_path} ...")
        try:
            edge_layer = GerberFile.open(edge_path)
            edge_segs = []
            for obj in edge_layer.objects:
                try:
                    for prim in obj.to_primitives(unit=MM):
                        geom = _primitive_to_shapely(prim)
                        if geom is not None and not geom.is_empty and geom.is_valid:
                            edge_segs.append(geom)
                except Exception:
                    pass
            if not edge_segs:
                raise ValueError("No se encontró geometría en Edge.Cuts.")
            edge_geom = unary_union(edge_segs)
            if cfg.MIRROR_X:
                edge_geom = mirror_geometry(edge_geom, cx)
            edge_geom = shapely_translate(edge_geom, xoff=-minx, yoff=-miny)

            output_stem = str(Path(output_path).with_suffix(''))
            edge_nc = f"{output_stem}-edge.nc"
            generate_edge_gcode(edge_geom, edge_nc, cfg=cfg)
            output_files.append(edge_nc)
            (progress_cb or print)(f"[✓] Edge cut → {edge_nc}")
        except Exception as e:
            (progress_cb or print)(f"[!] Edge.Cuts: {e}")
