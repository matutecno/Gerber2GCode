...
SAFE_Z_MM     = 3.000   # altura de viaje segura (solo fresa)
...
PASS_OVERLAP_FRAC = 0.4     # solapado entre pasadas
CUT_DEPTH_MM      = -0.05  # profundidad de corte (Z negativo)
PLUNGE_RATE       = 30     # velocidad de bajada (mm/min)
FEED_RATE         = 100     # velocidad de corte (mm/min)
...
def generate_slots_gcode(slots: list, output_path: str):
    ...
    for x1, y1, x2, y2, diam in slots:
        lines += [
            f"( slot ø{diam:.3f}mm )",
            f"G00 X{x1:.3f} Y{y1:.3f}",
            f"G01 F{SLOT_PLUNGE_RATE} Z{SLOT_DEPTH_MM:.3f}",
            f"G01 F{SLOT_FEED_RATE} X{x2:.3f} Y{y2:.3f}",
            f"G00 Z{DRILL_SAFE_Z_MM:.3f}",
        ]
    lines += ["M05", "M30"]
    with open(output_path, 'w') as f:
...
def process_drill_files(drl_paths: list, output_stem: str, cx: float = None):
...
def main():
    ...
    if drl_paths:
        ...
        print()
        process_drill_files(drl_paths, output_stem, cx=cx if MIRROR_X else None)
