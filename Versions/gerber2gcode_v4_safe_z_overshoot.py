...
CUT_DEPTH_MM      = -0.05  # profundidad de corte (Z negativo)
PLUNGE_RATE       = 60     # velocidad de bajada (mm/min)
FEED_RATE         = 900     # velocidad de corte (mm/min)
...
REF_MARK_DEPTH_MM = -0.15   # profundidad del punto V-bit (solo visible, no pasante)
...
def generate_ref_marks(board_w: float, board_h: float, output_stem: str):
    """
    Genera dos marcas de referencia con V-bit:
      P1 = origen (0, 0)
      P2 = esquina superior derecha (board_w, board_h)
    Produce:
      <stem>-ref.nc   → G-code para marcar los dos puntos
    ...
    lines = [
        ...
        f"( P1 = (0.000, 0.000)  —  origen )",
        f"( P2 = ({board_w:.3f}, {board_h:.3f})  —  esquina superior derecha )",
        "G17", "G21", "G54", "G90",
        ...
        f"G00 Z{SAFE_Z_MM:.3f}",
        f"( Marca 2 — esquina superior derecha )",
        f"G00 X{board_w:.3f} Y{board_h:.3f}",
        f"G01 F{PLUNGE_RATE} Z{REF_MARK_DEPTH_MM:.3f}",
        f"G00 Z{SAFE_Z_MM:.3f}",
        "M05", "M30",
    ]
    ...
    with open(txt_path, 'w') as f:
        ...
        f.write(f"EY2={board_h:.3f}\n")
        f.write(f"STEM={output_stem}\n")

    print(f"[ref] {nc_path}")
    print(f"      P1=(0.000, 0.000)  P2=({board_w:.3f}, {board_h:.3f})")
...
def main():
    ...
    if MODE == "mill":
        ...
        generate_mill_gcode(paths, output_path, clearance)
    ...
    output_stem = str(Path(output_path).with_suffix(''))
    generate_ref_marks(board_w, board_h, output_stem)
